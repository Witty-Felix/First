...
/* 全局变量 */
RS485_TypeDef g_RS485 = {0};
uint16_t RADAR_RX_DIST;
uint16_t RADAR_RX_TEMP;
/* 限幅滤波参数 */
#define LIMIT_THRESHOLD      50    /* 5.0cm（原始值单位0.1） */
/* 滑动平均参数 */
#define MA_WINDOW_SIZE       3

static uint16_t s_lastDist = 0;    /* 上次有效距离 */
static uint16_t s_lastTemp = 0;    /* 上次有效温度 */
static uint8_t  s_firstValid = 1;  /* 首次有效数据标志 */
static uint16_t s_maDist[MA_WINDOW_SIZE];   /* 距离滑动缓冲区 */
static uint16_t s_maTemp[MA_WINDOW_SIZE];   /* 温度滑动缓冲区 */

//灯带的数组：
 uint8_t rgb_send_cmd[21] = {
    0xDD,0x55,0xEE,     //帧头
    0x00,0x01,          //设备地址
...
//灯带实现的各种颜色
uint8_t rgb_no_light[3]     = 	{0x00,0x00,0x00};
uint8_t rgb_red_light[3]    =   {0xF0,0x00,0x00};
uint8_t rgb_blue_light[3]   = 	{0x00,0x00,0xff};
uint8_t rgb_green_light[3]  = 	{0x00,0xff,0x00};
uint8_t rgb_yellow_light[3] = 	{0xff,0xff,0x00};
uint8_t rgb_color_light[3]  =   {0xFF,0xFF,0xFF};   //自定义颜色
...
*************************************************************************************
*   函 数 名: Filter_Limit
*   功能说明: 限幅滤波 — 新值与上次有效值差值超过阈值则丢弃
*   形    参：new_val - 新采样值 | last_valid - 上次有效值
*   返 回 值: 滤波后的值
*   描    述：首次接收无条件接受；后续超过LIMIT_THRESHOLD则保持上次值
*************************************************************************************
*/
static uint16_t Filter_Limit(uint16_t new_val, uint16_t last_valid)
{
    int16_t diff;
    ...
    diff = (int16_t)(new_val - last_valid);
    if(diff < 0) diff = -diff;
        ...
        if(g_RS485.U1_RxCnt >= 5)
        {
            ...
            if(g_RS485.U1_RxCnt >= exp_len)
            {
                ...
                if(calc_crc == recv_crc)
                {
                    ...
                    /* 限幅滤波 */
                    RADAR_RX_DIST = Filter_Limit(RADAR_RX_DIST, s_lastDist);
                    RADAR_RX_TEMP = Filter_Limit(RADAR_RX_TEMP, s_lastTemp);
                    s_lastDist = RADAR_RX_DIST;
                    s_lastTemp = RADAR_RX_TEMP;
                    s_firstValid = 0;
...
void RS485_U1_ReadRegister(uint8_t slave_addr, uint16_t reg_addr, uint16_t reg_num)
{
    ...
    if(g_RS485.U1_Busy)
    {
        if((SysTick_ms - g_RS485.U1_TxTime) > 300)
        {
            g_RS485.U1_Busy = 0;
            g_RS485.U1_RxCnt = 0;
        }
